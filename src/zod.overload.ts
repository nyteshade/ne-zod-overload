...
import type {
  OverloadDecorator,
  OverloadParsedInputRow,
  Overloads,
  SafeParseReturnType,
} from './zod.overload.types';
 ...
 * }
 */
export const Overload: OverloadDecorator = function Overload(overloads: Overloads) {
  return function(target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = function(...args: any[]): any {
      const parsed = overloads
        ...
        .map(
          ([schema, handler, minArgs]): OverloadParsedInputRow => {
            let type = '';

            switch ((schema as ZodTypeAny)._def.typeName) {
              case z.ZodUndefined.name:
                type = 'no.params';
                break;
            ...
            const validated: OverloadParsedInputRow = {
              schema: schema as ZodTypeAny,
              safeParsed: undefined as SafeParseReturnType,
              handler,
            ...
            if (type === 'no.params') {
              ...
              validated.schema = z.custom(data => data.length === 0);
            } else {
              ...
              validated.schema = _schema.and(z.custom(_ => args.length === _minArgs));
            }
        ...
        .map(
          (element): OverloadParsedInputRow => {
            switch (element.type) {
              case 'no.params':
 ...
 * @extends Error
 */
export class NoMatchedHandlerError extends Error {
  /**
   ...
   */
  constructor(public overloads: Overloads, ...spaceJoinedStrings: string[]) {
